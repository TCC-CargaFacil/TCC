...
<body>
    ...
    <!-- Adicionei a verificação do id_mot aqui -->
    <?php
    session_start();

    // Verifica se o motorista está autenticado (presumindo que o ID do motorista está na sessão)
    if (!isset($_SESSION['idMotorista'])) {
        echo "Motorista não autenticado.";
        exit();
    }
    ...
    // Consulta para obter informações da tabela frete com base no id_mot
    $sql = "SELECT * FROM frete WHERE id_mot = ?";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "<td><a href='https://wa.me/{$row['telefone']}'><img src='Imagens/whatsapp.png' alt='#'></a></td>";
        ...
        echo "<p>Nenhuma corrida encontrada para o motorista logado.</p>";
    }

    // Feche a declaração preparada
    $stmt->close();
    ?>
    <!-- Fim da verificação do id_mot -->
